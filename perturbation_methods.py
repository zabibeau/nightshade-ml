...
def fgsm_penalty(source_tensor, target_latent, modifier, latent_function, eps=0.15):
...
def pgd_penalty(source_tensor, target_latent, modifier, latent_function, iterations=40, step_size=0.025, eps=0.15):
    modifier = modifier.detach().requires_grad_(True)
    source_tensor = source_tensor.detach().requires_grad_(True)
    target_latent = target_latent.detach()
...
def nightshade_penalty(source_tensor, target_latent, modifier, latent_function, t_size=150, eps=0.15):
    ...
    for i in range(t_size):
        ...
        modifier.detach().clone().requires_grad_(True)
        ...
        grad = torch.autograd.grad(loss, modifier, retain_graph=False)[0]
        ...
        if i % 50 == 0 or i == t_size - 1:
            print(f"Iteration {i}/{t_size} - Loss: {loss.item():.4f}")
